...
1. **Router pass (stage 1):** `LlmToolRouterAdapter` runs a two-pass, text-only LLM flow over a compact catalog (tool names + descriptions only — no schemas):
   - **Pass 1:** catalog only — initial primary tool picks.
   - **Pass 2 (refinement):** when pass 1 selects tools, prefix-scoped workflow skills from `llm/mcp-skills/` are injected (e.g. if pass 1 picks a `gitlab.*` tool, only the GitLab skill is appended). The router may add additional primary tools based on workflow guidance. Cross-server audit intent also bootstraps `jira` + `red` prefixes so cross-workflow skills are available. Pass 2 is skipped when pass 1 returns `NO_TOOLS_NEEDED`, `FAILED`, or no skills exist for the derived prefixes. Pass 2 failure falls back to pass 1's selection.
   - Returns a `ToolRouterResult` with an explicit outcome: `NO_TOOLS_NEEDED` (conversational turn → zero tools), `TOOLS_SELECTED`, or `FAILED` (fall back to all tools).
